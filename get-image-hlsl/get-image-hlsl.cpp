...
void CompileShader(_In_ LPCWSTR srcFile, _In_ LPCSTR entryPoint, _In_ LPCSTR profile, _Outptr_ ID3DBlob** blob)
{
	...
	*blob = nullptr;
	...
	if (FAILED(hr))
	{
		if (errorBlob)
		{
			// FIXME: This is incredibly stupid code and I can imagine no earthly reason
...
int main()
{
	...
	checkFail(Microsoft::WRL::Wrappers::RoInitializeWrapper initialize(RO_INIT_MULTITHREADED));
#else
	...
	checkFail(D3D11CreateDevice(nullptr,              /* Adapter: The adapter (video card) we want to use. We may use NULL to pick the default adapter. */
		...
		_context.GetAddressOf()));            /* OUT: the ID3D11DeviceContext that represents the above features. */
	...
	checkFail(_device->CreateVertexShader(
